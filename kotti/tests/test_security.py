...
class HasPermissionTests:
    def test_has_permission(self):
        ...
        assert (
            request.has_permission.assert_called_once_with(permission, context=context)
            is None
        )
...
class TestGroups:
    ...
    def test_nested_groups(self, db_session, root):
        ...
        assert set(list_groups("frank", root)) == {"group:franksgroup", "role:editor"}
        assert set(list_groups("frank", child)) == {"group:franksgroup", "role:editor"}
        ...
        assert set(groups) == {"group:bobsgroup", "group:franksgroup", "role:editor"}
        assert set(inherited) == {"group:bobsgroup", "group:franksgroup", "role:editor"}
    ...
    def test_works_with_auth(self, db_session, root):
        ...
        assert auth.effective_principals(request) == [  # user doesn't exist yet
            "system.Everyone"
        ]
    ...
    def test_principals_with_local_roles(self, db_session, root):
        ...
        assert set(principals_with_local_roles(child, inherit=False)) == {
            "group:bobsgroup"
        }
    ...
    def test_copy_local_groups(self, db_session, root):
        ...
        db_session.flush()
        assert set(principals_with_local_roles(child2)) == {"bob", "group:franksgroup"}
...
class TestPrincipals:
    ...
    def test_default_admin(self, db_session):
        ...
        assert self.get_principals().validate_password("secret", admin.password)
    ...
    def test_users_search(self, db_session):
        ...
        assert len(list(users.search(match="all", name="bob", active=True))) == 1
    ...
    def test_bobs_hashed_password(self, db_session):
        ...
        assert principals.validate_password("anothersecret", bob.password) is False
...
class TestHasPermission:
    def test_request_has_permission(self):
        ...
        with patch(
            "kotti.request.pyramid.request.Request.has_permission",
